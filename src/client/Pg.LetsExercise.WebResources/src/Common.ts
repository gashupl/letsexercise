export class CommonLib {
    ...
    constructor(context: any) {
        if (context !== null && context.getFormContext) {
            this.executionContext = context
            this.formContext = context.getFormContext()
        } else {
            this.formContext = context // for ribbon primary controls
        }
    }

    public getResourceString(key: string, webResourceName?: string) {
        if (!webResourceName) {
            webResourceName = 'sn_/resx/common'
        }
    ...
    public getWebApiconfig() {
        return { webApiVersion: '9.2', includeAnnotations: '*' }
    }
    ...
    public refreshRibbon(refreshAll: boolean | undefined) {
        if (
            this.formContext !== undefined &&
            this.formContext !== null &&
    ...
    public parseLookup(lookupName: string, webApiEntity: any): Array<Xrm.LookupValue> | null {
        ...
        var lookupValue: any[] | null = []
            ; (lookupValue[0] as any) = lookup

        return lookupValue
    }

    public convertOdataToLookup(odataEntity: { [x: string]: any }, fieldName: string): Array<Xrm.LookupValue> | null {
    ...
    public calculateNextDate(startDate: Date, numberofDay: number, onlyBusiness: any): Date {
    ...
    public setAttributeValue<T extends Xrm.Attributes.Attribute>(attributeName: string, value: Date | null) {
    ...
    public validateNip(fieldName: string) : boolean {
    ...
    public validatePesel(fieldName: string) : boolean {
        if (this.formContext.getAttribute(fieldName).getValue() != null) {
    ...
    public disableField(fieldName: string, disable: boolean) {
    ...
    public disableFields(fieldNames: string[], disable: boolean) {
    ...
    public disableFieldsWithSameName(fieldName: string, disable: boolean) {
    ...
    public hideFieldsWithSameName(fieldName: string, hide: any) {
        var attribute = this.getAttribute<Xrm.Attributes.Attribute>(fieldName);
        if (attribute) {
            attribute.controls.forEach((control: any) => {
    ...
    public clearTab(tabName: string, sectionName: number, shallNotClean: string) {
